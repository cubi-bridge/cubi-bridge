...
// El autoUpdater chequea https://github.com/cubi-bridge/cubi-bridge/releases
...
async function checkForUpdatesManual() {
  try {
    ...
    console.warn("[Bridge] checkForUpdates falló:", err?.message || err);
  }
}
...
function quitAndInstallUpdate() {
  try {
    ...
    console.warn("[Bridge] quitAndInstall falló:", err?.message || err);
  }
}
...
app.whenReady().then(() => {
  ...
  // Update check: electron-updater contra GitHub Releases. Al arrancar con
  // delay (no pisar la primera conexión) y cada 6h. Descarga automática.
  setupAutoUpdater();
  setTimeout(() => { autoUpdater.checkForUpdates().catch(() => {}); }, 10 * 1000);
